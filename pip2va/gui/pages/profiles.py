...
Two-column layout so plots aren't squashed vertically: the left column holds
the wire profile, phase-space images and emittance; the right column holds
the (large) 3D beam cloud, cycle sigma(s) and the Allison scanner. Controls
sit in compact bars. The 3D cloud is decimated for fast rendering."""
...
_CLOUD_MAX = 6000            # decimate the beam cloud to this many points
...
class ProfilesPage(Page):
    ...
    def build(self):
        ...
        # ---- top control bar: scanner selection + start (full width) ----
        bar = QHBoxLayout()
        ...
        self.chk_halo = QCheckBox("halo mode (7σ)")
        self.btn_scan = QPushButton("Start scan")
        self.lbl_fit = QLabel("fit: —")
        for w in (QLabel("Scanner:"), self.sel_ws, QLabel("points:"),
                  self.spin_pts, QLabel("pulses/pt:"), self.spin_ppp,
                  self.chk_halo, self.btn_scan):
            bar.addWidget(w)
        bar.addStretch(1)
        ...
        cloud_bar.addWidget(self.lbl_3d, 1)
        right.addLayout(cloud_bar)
        self.gl_view = None
        try:
            ...
            self.gl_scatter = gl.GLScatterPlotItem(size=2.0, pxMode=True)
            self.gl_view.addItem(self.gl_scatter)
            right.addWidget(self.gl_view, 4)
        except Exception as e:               # headless / no OpenGL context
            right.addWidget(QLabel(f"3D view unavailable: {e}"))
        ...
        alli = QHBoxLayout()
        alli.addWidget(QLabel("<b>Allison</b> (MEBT x-x′):"))
        self.btn_alli = QPushButton("Run scan")
        alli.addWidget(self.btn_alli)
        self.lbl_alli = QLabel("—")
        alli.addWidget(self.lbl_alli, 1)
        right.addLayout(alli)
        self.img_alli = pg.ImageView()
        self.img_alli.ui.roiBtn.hide()
        self.img_alli.ui.menuBtn.hide()
        self.img_alli.setMinimumHeight(180)
        right.addWidget(self.img_alli, 2)

        # ---- bottom control strip: cycle scans | scraper jaws (side by side)
        bottom = QHBoxLayout()
        bottom.setSpacing(12)
        cyc = QHBoxLayout()
        cyc.addWidget(QLabel("<b>Cycle</b>  wire pts:"))
        ...
        self.sp_wsq = QSpinBox(); self.sp_wsq.setRange(1, 20); self.sp_wsq.setValue(1)
        self.sp_lwq = QSpinBox(); self.sp_lwq.setRange(1, 20); self.sp_lwq.setValue(1)
        ...
        bottom.addLayout(cyc, 1)

        scr = QGridLayout()
        scr.addWidget(QLabel("<b>Scraper jaws</b> (mm / V / µA)"), 0, 0, 1, 4)
        self._scr_rows = {}
        names = [e.name for e in self.lat.elements if e.type == "scraper2"]
        for r_, nm in enumerate(names, start=1):
            scr.addWidget(QLabel(nm.split(":")[-1]), r_, 0)
    ...
    def _on_allison(self, _pid, data):
        ...
        self.lbl_alli.setText(
            f"ε={float(data['eps_ummrad'][0]):.3f}  "
            f"α={float(data['alpha'][0]):+.2f}  "
            f"β={float(data['beta_m'][0]):.2f} m"
            + ("  ✓" if float(data['done'][0]) else "  …"))
    ...
    def _render_cloud(self, cloud, station):
        try:
            pts = np.asarray(cloud, dtype=np.float32).T      # (n, 3) mm
            n_full = len(pts)
            if n_full > _CLOUD_MAX:                          # decimate: fast
                pts = pts[:: max(1, n_full // _CLOUD_MAX)]
            pts = pts.copy()
            ...
            self.lbl_3d.setText(
                f"{station} — {len(pts):,}/{n_full:,} shown "
                f"(z ×{span / zspan:.2g}; drag to rotate)")
